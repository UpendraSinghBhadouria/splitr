...
import { CreateExpenseInput, UserLite } from "@/types";
import { CategorySelector } from "./CategorySelector";
import { GetGroupOrMembers } from "@/types/groups";
...
export const ExpenseForm = ({
  ...
  const [selectedGroup, setSelectedGroup] =
    useState<GetGroupOrMembers["selectedGroup"]>(null);
  const [splits, setSplits] = useState<CreateExpenseInput["splits"]>([]);

  const { user: currentUser } = useUserSessionContext();

  const { create } = useServerAction(createExpense, {
    auto: false,
    method: "POST",
  });
      ...
      <div className="space-y-4">
        ...
        {type === "group" && (
          <div className="space-y-2">
            <Label>Group</Label>
            <GroupSelector
              onChange={(group: GetGroupOrMembers["selectedGroup"]) => {
