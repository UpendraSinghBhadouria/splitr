...
import { getGroupOrMembers } from "@/actions";
import { GetGroupOrMembers } from "@/types/groups";

export function GroupSelector({
  onChange,
}: Readonly<{
  onChange: (group: GetGroupOrMembers["selectedGroup"]) => void;
}>) {
  const [selectedGroupId, setSelectedGroupId] = useState("");

  const { data, isLoading, fetch } = useServerAction(getGroupOrMembers, {
    auto: false,
  });

  useEffect(() => {
    if (fetch) {
      fetch(selectedGroupId ? { groupId: selectedGroupId } : {});
    }
