...
import { CreateExpenseInput, SplitUIState, UserLite } from "@/types";
import { useUserSessionContext } from "@/hooks";

interface SplitSelectorProperties {
  type: "equal" | "percentage" | "exact";
  amount: number;
  participants: UserLite[];
  paidByUserId: string;
  onSplitsChange: Dispatch<SetStateAction<CreateExpenseInput["splits"]>>;
}
...
export const SplitSelector = ({
  ...
  paidByUserId,
  onSplitsChange,
}: SplitSelectorProperties) => {
  const [splits, setSplits] = useState<SplitUIState[]>([]);
