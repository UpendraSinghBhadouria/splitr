...
import { SettlementForm } from "@/components/client";
...
export default function SettlementPage() {
  ...
  const { fetch, data, isLoading } = useServerAction(getSettlementData, {
    auto: false,
  });
  ...
  useEffect(() => {
    if (fetch && type && id) {
      fetch({ entityType: type as "user" | "group", entityId: id as string });
    }
  }, [fetch, id, type]);
          ...
          <div className="flex items-center gap-3">
            {type === "user" ? (
              <Avatar className="h-10 w-10">
                <AvatarImage src={data?.counterpart?.image ?? ""} />
                <AvatarFallback>
